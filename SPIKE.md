...
- [ ] 잠금화면 위젯으로 기록 → 앱을 열면 그 기록이 보인다
- [ ] 앱에서 기록을 지움 → 잠금화면 위젯 표시가 따라서 바뀐다
- [ ] 앱에서 시작 기록 → 위젯에서 종료를 눌러 같은 기록에 종료일이 붙는다
- [ ] 앱과 위젯에서 같은 날을 각각 눌러도 기록이 하나만 남는다

1주차 스파이크는 UserDefaults 기준으로 이걸 확인했다. 2주차에 SwiftData로 바꾼 뒤로는
같은 수준의 확인을 다시 하지 않았다.
