...
- 확인 일자: 2026-09-02
- 기기 / iOS 버전: iPhone 17 / iOS 26.1
- **결과: 통과.** 잠금 상태에서 시작일·종료일 모두 앱을 열지 않고 기록되고,
  기록이 App Group을 통해 앱에서도 보인다. 1주차 스파이크의 전제가 성립함을 확인했다.
- 통과까지 걸린 문제 세 가지 (전부 코드 로직이 아니라 설정·환경 문제였다):
  1. Xcode가 팀 선택 과정에서 앱 타겟의 App Group 엔타이틀먼트를 비웠다 → 복구
  2. `authenticationPolicy` 기본값이 `.requiresAuthentication`이라 잠긴 기기에서 탭이 무시됐다 → `.alwaysAllowed`
  3. 앱을 재설치해도 잠금화면 위젯이 이전 익스텐션 바인딩을 유지했다 → 위젯 삭제 후 재추가
...
## 함정: 앱을 재설치해도 위젯은 갱신되지 않을 수 있다

앱을 다시 빌드해 설치해도, 잠금화면에 이미 올라가 있던 위젯이 **이전 익스텐션에 물린 채로 남아 있는 경우가 있다.**
버튼을 눌러도 반응이 없거나 옛날 동작을 그대로 하는데, 코드는 이미 고쳐진 상태라 원인을 찾기 어렵다.

위젯 관련 수정을 하고 나면 **잠금화면에서 위젯을 삭제했다가 다시 추가한다.**
스파이크 단계에서는 이걸 매번 하는 편이 낫다. 앱 삭제 → 재설치 → 위젯 재추가 순서면 확실하다.
