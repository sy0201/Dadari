...
이 함정은 1주차와 5~8주차에 두 번 걸렸다. 매번 "위젯 버튼이 먹통"으로 보여서 코드를
의심하게 되는데, 실제로는 위젯이 이전 빌드에 물려 있는 것이다.

앱 쪽에서 줄일 수 있는 만큼은 해뒀다. 앱이 앞으로 나올 때마다 `reloadAllTimelines()`를
불러 타임라인을 새로 만들게 한다. 다만 익스텐션 자체가 옛 바이너리에 물린 경우는
이걸로도 풀리지 않으므로, 위젯을 지웠다 다시 추가하는 절차는 여전히 필요하다.

**위젯 코드를 고쳤는데 동작이 안 바뀐다면 원인을 찾기 전에 이것부터 한다.**

### 왜 반복되는가

잠금화면에 올린 위젯은 우리가 만든 뷰가 아니라 **시스템이 관리하는 인스턴스**다.
사용자가 위젯을 추가하면 시스템은 `kind` 문자열로 위젯 정의를 찾아 인스턴스를 만들고,
렌더링된 스냅샷을 자기 쪽에 캐시한다. 익스텐션 프로세스도 시스템이 띄우고 재사용한다.

Xcode로 새 빌드를 설치하면 앱 번들은 교체되지만,

- 설정된 위젯 인스턴스는 그대로 남는다 (`kind`가 같으니 다시 붙일 이유가 없다)
- 시스템이 들고 있던 스냅샷 캐시가 곧바로 무효화되지 않는다
- 이미 떠 있던 익스텐션 프로세스가 옛 바이너리를 물고 있을 수 있다

그래서 **버튼을 눌러도 옛 코드가 돌거나 아무 일도 일어나지 않고, 화면도 다시 그려지지
않는다.** 오류도 로그도 남지 않아서 우리 코드를 의심하게 된다.
`kind`를 바꾸면 인스턴스가 새로 생기지만, 그건 사용자의 잠금화면에서 위젯이 사라진다는
뜻이라 개발 편의로 쓸 수 있는 방법이 아니다.

### 재발 방지로 넣은 것

원인이 개발 환경 쪽이라 없앨 수는 없다. 대신 **보이지 않던 실패를 보이게** 만들었다.

| 장치 | 하는 일 |
|---|---|
| `BuildStamp` | 실행 중인 번들 바이너리의 수정 시각. 빌드할 때마다 바뀐다 |
| 위젯 표시 (DEBUG) | 큰 문구 뒤에 빌드 시각을 붙인다. 방금 빌드한 시각과 다르면 옛 위젯이다 |
| 인텐트 로그 | 탭을 처리한 바이너리의 빌드 시각을 함께 남긴다 |
| 앱 포그라운드 진입 | `reloadAllTimelines()`로 타임라인을 다시 만들게 한다 |

앱과 위젯 익스텐션은 각자 자기 번들을 보므로 두 값이 따로 찍힌다.
**위젯에 찍힌 시각이 앱보다 오래됐으면 코드를 보지 말고 위젯을 지웠다 다시 추가한다.**

### 앞으로 위젯을 고칠 때의 순서

1. 빌드하고 설치한다
2. 잠금화면 위젯에 찍힌 빌드 시각이 방금 빌드한 시각과 같은지 본다
3. 다르면 위젯 삭제 → 재추가 (앱 재설치까지 필요할 때도 있다)
4. 같은데도 동작이 이상하면 그때부터 코드를 본다
5. Console.app에서 `subsystem:com.dadari.app`으로 인텐트 로그의 `빌드=` 값을 확인하면
   실제로 어느 바이너리가 탭을 처리했는지까지 가려진다
...
## 유닛 테스트로 확인할 수 없는 것: 프로세스 간 SwiftData 가시성

앱과 위젯 익스텐션은 **별개 프로세스에서 각자 `ModelContainer`를 만들어** App Group 안의
같은 스토어 파일을 연다. 한쪽에서 쓰거나 지운 것이 다른 쪽에 곧바로 보이는지는
잠금화면 기록의 전제 조건인데, **유닛 테스트로는 검증할 수 없다.**

한 프로세스 안에서 컨테이너 두 개로 같은 파일을 열어 흉내내 봤지만, 두 번째 컨테이너에서
쓰기를 하면 `NSCocoaErrorDomain 134020`("모델 구성이 호환되지 않습니다")이 난다.
프로세스 안에서 컨테이너를 두 개 여는 것 자체가 지원되지 않는 구성이라, 이 실패는
앱↔위젯 동작이 아니라 테스트 방식의 한계를 보여줄 뿐이다. 그래서 그 테스트는 걷어냈다.

**실기기 확인 결과 (iPhone 17 / iOS 26.1): 통과.**

- [x] 잠금화면 위젯으로 기록 → 앱을 열면 그 기록이 보인다
- [x] 앱에서 기록을 지움 → 잠금화면 위젯 표시가 따라서 바뀐다

1주차 스파이크는 UserDefaults 기준으로 이걸 확인했고, 2주차에 SwiftData로 바꾼 뒤로는
확인이 비어 있었다. App Group 안의 스토어 파일을 두 프로세스가 각자 컨테이너로 열어도
서로의 변경이 보인다는 것을 실기기에서 확인했다. `PeriodRecordStore`가 호출마다
`ModelContext`를 새로 만드는 구조가 여기에 맞게 동작한다.
