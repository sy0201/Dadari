...
/// 애플 건강 앱과 같은 방식이다.
///
/// 처음에는 "탭한 날짜를 시작점으로 하는 일주일"로 잡았지만, 그렇게 하면 고른 날짜가
/// 늘 맨 왼쪽으로 오면서 요일 열이 매번 어긋난다. 같은 주 안에서 날짜를 옮길 때마다
/// 격자가 흔들려 위치를 가늠하기 어려웠다. 실제로 써 본 뒤 달력 주 고정으로 바꿨고,
/// PRD 3.2와 목업(ui-mockup.html)도 그에 맞춰 갱신했다.
